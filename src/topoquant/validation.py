...
from .data import STOCK_NAME, list_stock_files, read_csv_flexibly, stock_code_from_path
...
def validate_dataset(
    ...
    if worker_count == 1:
        results = map(lambda path: _validate_one(path, config), files)
        executor = None
    else:
    ...
    report_path = config.output_dir / "data_validation.csv"
